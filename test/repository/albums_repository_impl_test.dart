...
import 'package:modu_3_dart_study/repository/albums_repository_impl.dart';
...
void main() {
  ...
  final DataSource albumDataSource = MockDataSourceImpl(json);
  final AlbumsRepository repository = AlbumsRepositoryImpl(albumDataSource);

  test('limit 값이 없는 경우 전체 리스트가 조회된다', () async {
    final noLimit = await repository.getAlbums();
    expect(noLimit.length, 100);
  });

  test('limit 값이 있는 경우 해당 리스트까지 조회된다', () async {
    // 경계값 테스트
    final limits = [0, 1, 2, 99, 100, 101];
    final expectedLength = [0, 1, 2, 99, 100, 100];


    for (int i = 0; i < limits.length; i ++) {
      final limit = limits[i];
      final albums = await repository.getAlbums(limit: limit);
      expect(albums.length, expectedLength[i]);
    }
  });
}
